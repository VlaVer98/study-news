...
class UploadImage {
	...
	public function check_type($name_file, $src, $type) {
		...
		if($matches[0] == 'jpeg') $matches[0] = 'jpg';

		if(!list($w, $h) = @getimagesize($src)){
			$this->err[] = "{$name_file} не является изображением!";
			return false;
		}
		...
		switch($matches[0]){
			...
			default : $this->err[]="{$name_file} имеет не поддеживаемый формат(разрешены: .bmp, .gif, .jpg, .png)"; return false;
		}
	}
	...
	public function max_size($name_file, $size) {
		...
		$this->err[] = "Изображение {$name_file} должно быть не больше {$this->max_size} Мбайт";
	}

	public function generated_name() {
		$name = uniqid("", true);
		
		if(\R::findOne('gallery', "file_name=?", [$name])) {
	...
	*/
	public function save($file, $max_size=null, $dst = "img/gallery/") {
		$this->file = $file;
