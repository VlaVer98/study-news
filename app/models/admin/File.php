...
class File {
	...
	public function get($id_category) {
		return \R::getAll("SELECT * FROM file WHERE category_id=?", [$id_category]);
	}
	public function delete($id_file) {
		$file = \R::load('file', $id_file);
		if($file) {
			$file_name = \R::getCell("SELECT file_name FROM file WHERE id=?", [$id_file]);
			\R::trash($file);
			unlink(WWW."/docs/{$file_name}");
			return true;
		}
		return false;
	}
	...
	protected function check_unique($id_category) {
		...
		$file_db = \R::getAll("SELECT title FROM file WHERE category_id=? AND title IN (". \R::genSlots($names) .")", \R::flat(array($id_category, $names)));

		if($file_db) {
			foreach ($file_db as $key => $value) {
				$this->err[] = "Файл с именем {$value['title']} уже существует в данной категории";
			}
			return false;
		} else {
			return true;
